 ...
 */
public class MaxDistance_0317 {
    static class MaxDistanceInfo_0317 {
        private int maxDis;
        private int height;

        public MaxDistanceInfo_0317(int maxDis, int height) {
            this.maxDis = maxDis;
            this.height = height;
        }
    }
    ...
    private static MaxDistanceInfo_0317 recurse(TreeNode x) {
        if(x == null) {
            return new MaxDistanceInfo_0317(0,0);
        }

        MaxDistanceInfo_0317 leftInfo = recurse(x.left);
        MaxDistanceInfo_0317 rightInfo = recurse(x.right);

        int height = Math.max(leftInfo.height, rightInfo.height) + 1;
        int maxDis = Math.max(Math.max(leftInfo.maxDis, rightInfo.maxDis), leftInfo.height + rightInfo.height + 1);
        return new MaxDistanceInfo_0317(maxDis, height);
    }
