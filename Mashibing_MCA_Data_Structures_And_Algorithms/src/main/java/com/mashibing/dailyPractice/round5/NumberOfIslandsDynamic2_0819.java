 ...
 */
public class NumberOfIslandsDynamic2_0819 {
    ...
    public NumberOfIslandsDynamic2_0819(int m, int n) {
        if(m <=0 || n <= 0) {
            return;
        }
    ...
    public boolean hasExisted(int i, int j) {
        int anc = findAnc(findIndex(i, j));
        return sizeArr[anc] != 0;
    }
    ...
    public static List<Integer> numberOfIslandsDynamic2(int m, int n, int[][] positions) {
        ...
        if(positions == null || positions.length == 0 || positions.length > m || positions[0].length > n) {
            return list;
        }

        NumberOfIslandsDynamic2_0819 uf = new NumberOfIslandsDynamic2_0819(m, n);
        for (int[] position : positions) {
            int i = position[0];
            int j = position[1];
            if(uf.hasExisted(i, j)) {
                list.add(uf.size());
                continue;
            }

            uf.connect(i, j);
            if(j + 1 < n && uf.hasExisted(i, j + 1)) {
                uf.union(i, j, i, j + 1);
            }
            if(j - 1 >= 0 && uf.hasExisted(i, j - 1)) {
                uf.union(i, j, i, j - 1);
            }
            if(i - 1 >= 0 && uf.hasExisted(i - 1, j)) {
                uf.union(i, j, i - 1, j);
            }
            if(i + 1 < m && uf.hasExisted(i + 1, j)) {
                uf.union(i, j, i + 1, j);
            }
