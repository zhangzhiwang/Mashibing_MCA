...
import com.mashibing.array.PreSum;
...
public class CardsInLine_0815 {
    ...
    public static int cardsInLine2(int[] arr) {
        ...
        for (int j = 1; j < n; j++) {
            for (int i = j - 1; i >=0; i--) {
                int p1 = arr[i] + buf2[i + 1][j];
                int p2 = arr[j] + buf2[i][j - 1];
                buf1[i][j] = Math.max(p1, p2);

                p1 = buf1[i + 1][j];
                p2 = buf1[i][j - 1];
                buf2[i][j] = Math.min(p1, p2);
            }
        }
