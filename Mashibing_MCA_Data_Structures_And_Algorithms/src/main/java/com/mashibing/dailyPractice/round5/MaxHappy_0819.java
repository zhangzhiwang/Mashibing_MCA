...
/**
 * 一棵多叉树代表一个公司的组织架构，现在公司搞聚会，给员工发请柬，规则是如果给某个员工发了请柬那么就不能给他的直接上级和直接下级发请柬，每个员工都有自己的快乐值happy，
 * 收到请柬的员工去参加聚会，求参加聚会的员工快乐值总和的最大值，其中快乐值是非负数。注意：同级别的同事之间没有限制，即邀请了A，和他同级的B可能被邀请也可能不会被邀请。
 */
public class MaxHappy_0819 {
    static class MaxHappyInfo_0819 {
        private int invited;
        private int notInvited;

        public MaxHappyInfo_0819(int invited, int notInvited) {
    ...
    public static int maxHappy(NaryTreeNode<Integer> root) {
        ...
        MaxHappyInfo_0819 info = recurse(root);
        return  Math.max(info.invited, info.notInvited);
    }

    private static MaxHappyInfo_0819 recurse(NaryTreeNode<Integer> root) {
        if(root == null) {
            return new MaxHappyInfo_0819(0, 0);
        }

        int invited = root.data;
        int notInvited = 0;
        for (NaryTreeNode<Integer> child : root.children) {
            MaxHappyInfo_0819 info = recurse(child);
            invited += info.notInvited;
            notInvited += Math.max(info.invited, info.notInvited);
        }

        return new MaxHappyInfo_0819(invited, notInvited);
    }
