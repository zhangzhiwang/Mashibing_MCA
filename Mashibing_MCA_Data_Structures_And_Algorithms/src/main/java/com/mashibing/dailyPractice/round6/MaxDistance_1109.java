 ...
 */
public class MaxDistance_1109 {
    static class MaxDistanceInfo_1109 {
        private int maxDis;
        private int height;

        public MaxDistanceInfo_1109(int maxDis, int height) {
            this.maxDis = maxDis;
            this.height = height;
        }
    }
    ...
    private static MaxDistanceInfo_1109 recurse(TreeNode<Integer> root) {
        if(root == null) {
            return new MaxDistanceInfo_1109(0, 0);
        }

        MaxDistanceInfo_1109 left = recurse(root.left);
        MaxDistanceInfo_1109 right = recurse(root.right);
        int p1 = Math.max(left.maxDis, right.maxDis);
        int p2 = left.height + right.height + 1;

        int maxDis = Math.max(p1, p2);
        int height = Math.max(left.height, right.height) + 1;
        return new MaxDistanceInfo_1109(maxDis, height);
    }
