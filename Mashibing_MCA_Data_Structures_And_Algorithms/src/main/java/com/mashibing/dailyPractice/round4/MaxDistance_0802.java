 ...
 */
public class MaxDistance_0802 {
    static class MaxDistanceInfo_0802 {
        private int maxDis;
        private int height;

        public MaxDistanceInfo_0802(int maxDis, int height) {
            this.maxDis = maxDis;
            this.height = height;
        }
    }
    ...
    private static MaxDistanceInfo_0802 recurse(TreeNode<Integer> x) {
        if(x == null) {
            return new MaxDistanceInfo_0802(0, 0);
        }

        MaxDistanceInfo_0802 left = recurse(x.left);
        MaxDistanceInfo_0802 right = recurse(x.right);
        int p1 = Math.max(left.maxDis, right.maxDis);
        int p2 = left.height + right.height + 1;
        int maxDis = Math.max(p1, p2);
        int height = Math.max(left.height, right.height) + 1;
        return new MaxDistanceInfo_0802(maxDis, height);
    }
