 ...
 * 返回你能装下最多的价值是多少?
 */
public class Knapsack_0512 {
    public static int knapsack1(int[] weights, int[] values, int bag) {
        if(weights == null || values == null || weights.length != values.length || weights.length == 0 || bag < 1) {
            return 0;
        }

        return recurse1(weights, values, 0, bag);
    }

    private static int recurse1(int[] weights, int[] values, int index, int rest) {
        if(rest < 0) {
            return -1;
        }
        ...
        int p1 = recurse1(weights, values, index + 1, rest);
        int p2 = 0;
        int value = recurse1(weights, values, index + 1, rest - weights[index]);
        if(value >= 0) {
