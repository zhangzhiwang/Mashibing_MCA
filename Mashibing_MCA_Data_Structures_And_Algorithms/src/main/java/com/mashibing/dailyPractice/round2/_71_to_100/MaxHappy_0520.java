 ...
 */
public class MaxHappy_0520 {
    static class MaxHappyInfo_0520 {
        private int invited;
        private int notInvited;

        public MaxHappyInfo_0520(int invited, int notInvited) {
    ...
    public static int maxHappy(NaryTreeNode<Integer> head) {
        ...
        MaxHappyInfo_0520 maxHappyInfo_0520 = recurse(head);
        return Math.max(maxHappyInfo_0520.invited, maxHappyInfo_0520.notInvited);
    }

    private static MaxHappyInfo_0520 recurse(NaryTreeNode<Integer> x) {
        if(x == null) {
            return new MaxHappyInfo_0520(0, 0);
        }

        int invited = x.data;
        int notInvited = 0;
        for (NaryTreeNode<Integer> child : x.children) {
            MaxHappyInfo_0520 childInfo = recurse(child);
            invited += childInfo.notInvited;
            notInvited += Math.max(childInfo.invited, childInfo.notInvited);
        }

        return new MaxHappyInfo_0520(invited, notInvited);
    }
