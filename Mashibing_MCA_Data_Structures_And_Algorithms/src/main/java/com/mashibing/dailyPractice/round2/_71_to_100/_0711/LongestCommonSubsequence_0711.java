 ...
 */
public class LongestCommonSubsequence_0711 {
    public int longestCommonSubsequence1(String text1, String text2) {
        if(text1 == null || text1.equals("") || text2 == null || "".equals(text2)) {
            return 0;
        }
    ...
    private static int recurse(char[] charArr1, int i, char[] charArr2, int j) {
        if(i == 0 && j == 0) {
    ...
    public int longestCommonSubsequence2(String text1, String text2) {
        ...
        int[][] buf = new int[N][M];
        buf[0][0] = charArr1[0] == charArr2[0] ? 1: 0;

        for (int i = 1; i < M; i++) {
            buf[0][i] = charArr1[0] == charArr2[i] ? 1 : buf[0][i - 1];
        }

        for (int i = 1; i < N; i++) {
            buf[i][0] = charArr1[i] == charArr2[0] ? 1 : buf[i - 1][0];
        }
        ...
        for (int j = 1; j < M; j++) {
            for (int i = 1; i < N; i++) {
                int p1 = buf[i - 1][j];
                int p2 = buf[i][j - 1];
                int p3 = charArr1[i] == charArr2[j] ? 1 + buf[i - 1][j - 1] : 0;
