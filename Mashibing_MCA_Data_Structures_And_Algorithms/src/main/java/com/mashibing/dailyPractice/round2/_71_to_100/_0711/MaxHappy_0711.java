 ...
 */
public class MaxHappy_0711 {
    static class MaxHappyInfo_0711 {
        private int invited;
        private int notInvited;

        public MaxHappyInfo_0711(int invited, int notInvited) {
    ...
    public static int maxHappy(NaryTreeNode<Integer> root) {
        ...
        MaxHappyInfo_0711 info = recurse(root);
        return Math.max(info.invited, info.notInvited);
    }

    private static MaxHappyInfo_0711 recurse(NaryTreeNode<Integer> x) {
        if (x == null) {
            return new MaxHappyInfo_0711(0, 0);
        }

        int invited = x.data;
        int notInvited = 0;
        for (NaryTreeNode<Integer> child : x.children) {
            MaxHappyInfo_0711 childInfo = recurse(child);
            invited += childInfo.notInvited;
            notInvited += Math.max(childInfo.invited, childInfo.notInvited);
        }

        return new MaxHappyInfo_0711(invited, notInvited);
    }
