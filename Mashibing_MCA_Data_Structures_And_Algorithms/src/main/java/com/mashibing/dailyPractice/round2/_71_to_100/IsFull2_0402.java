 ...
 * 判断一棵树是不是满二叉树
 */
public class IsFull2_0402 {
    static class IsFull2Info_0402 {
        private boolean isFull;
        private int height;

        public IsFull2Info_0402(boolean isFull, int height) {
            this.isFull = isFull;
            this.height = height;
        }
    }
    ...
    private static IsFull2Info_0402 recurse(TreeNode head) {
        if(head == null) {
            return new IsFull2Info_0402(true, 0);
        }

        IsFull2Info_0402 leftInfo = recurse(head.left);
        IsFull2Info_0402 rightInfo = recurse(head.right);
        ...
        return new IsFull2Info_0402(isFull, height);
    }
