 ...
 */
public class MaxDistance_0402 {
    static class MaxDistanceInfo_0402 {
        private int maxDis;
        private int height;

        public MaxDistanceInfo_0402(int maxDis, int height) {
            this.maxDis = maxDis;
            this.height = height;
        }
    }
    ...
    private static MaxDistanceInfo_0402 recurse(TreeNode head) {
        if(head == null) {
            return new MaxDistanceInfo_0402(0, 0);
        }

        MaxDistanceInfo_0402 leftInfo = recurse(head.left);
        MaxDistanceInfo_0402 rightInfo = recurse(head.right);

        int height = Math.max(leftInfo.height, rightInfo.height) + 1;
        int maxDis = Math.max(Math.max(leftInfo.maxDis, rightInfo.maxDis), leftInfo.height + rightInfo.height + 1);
        return new MaxDistanceInfo_0402(maxDis, height);
    }
