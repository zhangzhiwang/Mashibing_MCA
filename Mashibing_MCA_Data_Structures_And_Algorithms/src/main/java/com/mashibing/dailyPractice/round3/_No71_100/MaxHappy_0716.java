 ...
 * 每个员工都有自己的快乐值happy，收到请柬的员工去参加聚会，求参加聚会的员工快乐值总和的最大值，其中快乐值是非负数。注意：同级别的同事之间没有限制，
 * 即邀请了A，和他同级的B可能被邀请也可能不会被邀请。
 */
public class MaxHappy_0716 {
    static class MaxHappyInfo_0716 {
        private int invited;
        private int notInvited;

        public MaxHappyInfo_0716(int invited, int notInvited) {
    ...
    public static int maxHappy(NaryTreeNode<Integer> root) {
        ...
        MaxHappyInfo_0716 info = recurse(root);
        return Math.max(info.invited, info.notInvited);
    }

    private static MaxHappyInfo_0716 recurse(NaryTreeNode<Integer> x) {
//        if(x == null) {
//            return new MaxHappyInfo_0716(0, 0);
//        }

        int invited = x.data;
        int notInvited = 0;
        for (NaryTreeNode child : x.children) {
            MaxHappyInfo_0716 childInfo = recurse(child);
            invited += childInfo.notInvited;
            notInvited += Math.max(childInfo.invited, childInfo.notInvited);
        }

        return new MaxHappyInfo_0716(invited, notInvited);
    }
