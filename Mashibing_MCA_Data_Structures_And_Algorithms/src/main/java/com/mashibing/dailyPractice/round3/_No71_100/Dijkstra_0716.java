...
public class Dijkstra_0716 {
    public static Map<GraphNode<String>, Integer> dijkstra(GraphNode<String> node) {
        ...
        while (minDisNode != null) {
            int dis = map.get(minDisNode);
            List<Edge<String>> edges = minDisNode.edges;
            for (Edge<String> edge : edges) {
                GraphNode<String> toNode = edge.to;
                if(map.containsKey(toNode) && dis + edge.weight >= map.get(toNode)) {
                    continue;
                }
                map.put(toNode, dis + edge.weight);
            }
    ...
    private static GraphNode<String> findMinDisNode(Map<GraphNode<String>, Integer> map, Set<GraphNode<String>> set) {
        ...
        for (GraphNode<String> n : map.keySet()) {
            if(set.contains(n)) {
                continue;
            }
