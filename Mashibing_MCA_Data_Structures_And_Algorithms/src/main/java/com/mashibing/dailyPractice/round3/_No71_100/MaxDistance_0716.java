...
/**
 * 给定一颗二叉树head，任意两个节点之间都会有距离，每经过一个节点距离加1，最短的距离是节点自己和自己的距离，距离是1，
 * 每个节点和其相邻节点的距离是2，求该树的最大距离
 */
public class MaxDistance_0716 {
    static class MaxDistanceInfo_0716 {
        private int maxDis;
        private int height;

        public MaxDistanceInfo_0716(int maxDis, int height) {
            this.maxDis = maxDis;
            this.height = height;
        }
    }
    ...
    private static MaxDistanceInfo_0716 recurse(TreeNode x) {
        if(x == null) {
            return new MaxDistanceInfo_0716(0, 0);
        }

        MaxDistanceInfo_0716 left = recurse(x.left);
        MaxDistanceInfo_0716 right = recurse(x.right);
        int p1 = Math.max(left.maxDis, right.maxDis);
        int p2 = left.height + right.height + 1;
        int maxDis = Math.max(p1, p2);
        int height = Math.max(left.height, right.height) + 1;
        return new MaxDistanceInfo_0716(maxDis, height);
    }
