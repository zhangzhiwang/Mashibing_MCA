 ...
 * 所以，辅助的容器就不能用FIFO的队列了，得用栈，因为DFS还要原路返回，而BFS就不需要。
 * 整个过程要注意几点：
 * 1、打印的时机要在入栈的时候，而不是出栈的时候，因为一个节点要入栈出栈多次，而出栈是无条件的，入栈是有条件的，条件就是该节点的其中一个邻居节点不在Set中，
 * 所以如果出栈打印，会打印重复数据。换句话说，入栈时打印是在第一次遍历到该节点时打印，出栈打印不能保证是第一次出栈时打印。怎么判断某节点是第一次被遍历到呢？
 * 答案就是当set不存在该节点的时候就是第一次被遍历到。
 * 2、在把一个节点的邻居节点压入栈之前，先把该节点入栈，这一步不能省且顺序不能错，因为栈里从栈头到栈尾保存的是一条完整的路径，后续还要倒序退回来，靠的就是这个栈。
 * 假如A的邻居节点是B，遍历的时候是先遍历A再遍历B，等到最后回退的时候是先遍历B后遍历A，所以在正序遍历的时候必须先把A入栈，再把邻居B入栈，回退的时候顺序正好相反。
 * 如果A有好多邻居（出度），就会发现A会被入栈出栈好多次，这也是为什么要在第一次入栈时打印的原因。
 *
 * 课程：体系班课时142
 */
public class DFS {
