...
public class Knapsack {
    ...
    private static int recurse(int[] weights, int[] values, int rest, int index) {
        /*
         base case：base case有两个：一个是没有东西可以选了，一个是数书包没有剩余重量额度了。
         要先判断rest后判断index，不能反，说白了就是先看看书包还能不能装，再看还有没有东西可装。
         为什么不能反？
         考虑一个特殊情况，也会是边界情况：就是剩余最后一个物品时如果要了就超重了。
         分析：
         现在剩余最后一个物品，重量是15，但是bag剩余可装重量为10，现在有两种选择：要和不要。
         如果要了就超重了，给下一次递归传入的参数是index=weights.length，bag=-5。
         如果先判断index是不是等于weights.length，那么这种情况符合判断条件，就给上层返回了0，言外之意告诉上层：你可以要最后一个物品；
         但是如果先判断bag就会发现已经超重了，返回-1，言外之意告诉上层：最后一个你不能要，因为已经超重了。
         所以，必须先判断bag是否超重。
         */
        // base case1
        /*
