...
public class Knapsack {
    ...
    private static int recurse(int[] weights, int[] values, int rest, int index) {
        // base case：base case有两个：一个是没有东西可以选了，一个数书包没有剩余重量额度了，要先判断rest后判断index，不能反
        // base case2
        /*
        当书包剩余能装的重量为0时，说明已经装满了，当前考察的index位置的物品肯定不能要了。
        但是跟base case1不一样的是此种情况不能返回0，要返回-1，原因就是告诉上游调用的地方：此种方案不能选否则书包超载，所以要返回一个负数的无效值
         */
        if(rest < 0) {
            return -1;
        }
...
//        int p2 = values[index] + recurse(weights, values, rest - weights[index], index + 1);// 如果要了index的物品的情况，不能这么写，原因在下面
        ...
        但是我在本轮递归中没有发现超载了，会继续调用递归函数考察后面的位置，但是考查后面位置的物品时发现rest已经小于0了，命中了base case2，base case2返回了-1，
        就是告诉上游：哥哥，你上一个物品就不应该要，因为你要了就超标了，这也就是为什么base case2不能返回0的原因，因为如果这种情况下游递归函数返回了0，上游是无感的，
        上游会以为是到了数组的尽头，如果校友返回一个无意义的值，那么上游就知道了：我选了index就超标了，所以不能选。
         */
        int v = recurse(weights, values, rest - weights[index], index + 1);// 先让校友判断一下是不是超标了
        int p2 = 0;
    ...
    跳过版本二，通过版本一直接改出版本三：
    1、既然(rest,index)对递归产生影响，那么如果有版本二的话也是通过(rest,index)建立二维数组，那么不妨直接分析这两个变量。
