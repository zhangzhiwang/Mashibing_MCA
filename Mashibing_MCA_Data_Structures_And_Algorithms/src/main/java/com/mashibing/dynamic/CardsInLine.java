...
public class CardsInLine {
    ...
    private static int g1(int[] arr, int L, int R) {
        ...
        /*
         ...
         同时轮到后手选择的时候，后手又变成了下一轮的先手，他会和对手使用相同的策略：选择一个对自己最有力的，留给对手最不利的，
    ...
    public static int cardsInLine2(int[] arr) {
        ...
        // 准备两个个大小为N*N的二维数组作为缓存并将值初始化为无意义的-1
        int N = arr.length;
    ...
    在代码中，f(L,R)要依赖g(L+1,R)和g(L,R-1)，g(L,R)要依赖f(L+1,R)和f(L,R-1)，所以可以得出结论：在先手二维矩阵中的任意一个格子x，它依赖于后矩阵中相同位置格子（_x）的右边和下边，
    ...
    4、看顺序
    看自己在草稿纸上填个字的上下左右顺序。本题的填入顺序是斜着往右上填的，所以遍历的时候就要斜着往右上遍历
     */
    public static int cardsInLine3(int[] arr) {
        ...
        for(int R = 1; R < N; R++) {// 分析一下二维表的特点：两个二维表对角线已经填好了，右下部分不用填，可以看出列实际上是从1开始往后走的
            int _R = R;// 注意：由于内存循环需要列++，网曾循环也要列++，两个列++是控制不同的东西的，不能串了，所以拷贝一个_R出来，内层循环使用_R，这样外层循环不受干扰
