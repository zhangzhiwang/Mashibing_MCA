...
public class StickersToSpellWord {
    ...
    private static Map<String, Integer> recurse1(String[] stickers, String rest) {
        int minCount = Integer.MAX_VALUE;// 除当前纸牌外，后续使用纸牌张数的最小值
        String remaining = "";
        for (int i = 0; i < stickers.length; i++) {
            ...
            if(map.get("restStrLength") == 0) {
                ...
                remaining = "";
                minCount = Math.min(minCount, map.get("count") + 1);// minCount是不算当前指派的最小值张数，算上当前张要加上1
            }
        }
    ...
    private static String restStr(String target, String sticker) {
        ...
        /*
         ...
         注意：要弄清是谁消谁，是sticker去消target，所以sticker数组++，target数组--，不要搞反，否则成了target去消sticker了
         */
    ...
    // ---------------版本二：方案一的优化版（仍为暴力递归）---------------
    /*
    该优化版本和方案一的整体思路是一样的，优化点有两处：
    1、把原始贴纸和target字符串都搞成词频统计的形式
    2、剪枝
    思路比较绕，相对复杂一些，真正面试时如果遇到此题写出方案一即可，由于时间关系此版本的实现忽略，待以后补充。
    该版本的课程：体系班课时190-191
    ...
    即使是转成字符数组那么也不方便填充每一个格子，巨费事，所以舍弃原本的最终版，将傻缓存版就定为最终版。
     */
