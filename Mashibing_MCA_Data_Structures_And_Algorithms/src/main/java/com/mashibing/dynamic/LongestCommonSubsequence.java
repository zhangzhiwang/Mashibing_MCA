...
public class LongestCommonSubsequence {
    ...
    private static int recurse1(char[] str1Arr, int i, char[] str2Arr, int j) {
        if(i == 0 && j == 0) {// base case1：当这两个字符序列都只有一个字符，那么只判断这两个字符是不是相等即可，如果相等最长子序列的长度就是1
            ...
            /*
             ...
             2、不一定以j位置的字符结束，说明公共子序列可能包含j位置的字符，那么j位置的字符就系需要考查
             */
    ...
    /*
    跳过版本二，直接版本三。
    分析：通过版本一的递归过程可以看到只有两个变量i和j影响整个递归过程，所以可以使用一个二维时序来表示：横坐标是i，纵坐标是j。
    ...
    至此，第一行和第一列都填满了
    4、最后看普适位置的格子怎么填，也就是上面p1、p2和p3的情况，可以看出任意一个普适格子[i][j]的值要么取决于它左面的格子，要么取决于它右面的格子，要么取决于它左下的格子
