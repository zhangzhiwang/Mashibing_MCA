 ...
 * 1、最长公共子序列一定不以i位置的字符结尾，但不一定以j位置的字符结尾，即一定要考虑i位置但不一定要考虑j位置
 * 2、最长公共子序列一定不以j位置的字符结尾，但不一定以i位置的字符结尾，即一定要考虑j位置但不一定要考虑i位置
 * 3、最长公共子序列一定是以i位置的字符结尾同时也以j位置的字符结尾，所以这种情况下i和j位置的字符是一样的，即str1[i]=str2[j]
 *
...
public class LongestCommonSubsequence {
     ...
     *        注意：这里的i和j不是考查的是第i个字符和第j个字符，是第0-i个字符和第0-j个字符
     * @return
     */
    private static int recurse1(char[] str1Arr, int i, char[] str2Arr, int j) {
        if(i == 0 && j == 0) {// base case1：当这两个字符序列都只有一个字符，那么只判断这两个字符是不是相等即可，如果相等最长子序列的长度就是1
            ...
            /*
             情况1：公共子序列一定不以i位置的字符结束，不一定以j位置的字符结束
             1、公共子序列一定不以i位置字符结尾，说明i位置不用考察了，从i-1位置往前考查即可
             2、不一定以j位置的字符结束，说明公共子序列可能包含j位置的字符也可能不包含，那么j位置的字符就需要考查
             */
            int p1 = recurse1(str1Arr, i - 1, str2Arr, j);

            // 情况2：公共子序列一定是不以j位置的字符结束，不一定以i位置的字符结束，所以i位置必须考查，j位置就不用考查了，从j-1位置往前考查即可
