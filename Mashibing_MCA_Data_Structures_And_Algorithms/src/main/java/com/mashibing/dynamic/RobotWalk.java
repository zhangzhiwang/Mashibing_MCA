...
public class RobotWalk {
     ...
     */
    private static int recurse1(int cur, int target, int rest, int N) {// 注意：当参数有很多时可以封装成一个类，如果不封装，而且参数相同类型的有很多时，注意不要传错，测试时就将target和rest传反了
        /*
        ...
        } else if(cur == N) {// 从代码的形式上拉看，当cur=N和cur处在1-N中间的某个位置时是可以合并在一起的，但是这样代码不是很好理解
    ...
    /*
    思路：
    通过分析recurse1可以发现，recurse1的入参中target和N这两个参数在每次递归调用中都没有发生过变化，把最初用户传进来的这两个值一路传递下去，
    ...
    这里就要注意了：横坐标要放下N个位置，而题目说的位置是从1开始的，为了能放下这1-N个位置而且还要让这1-N做下标，所以横坐标要申请N+1个长度，索引范围是[0,N],
    ...
    public static int robotWalk2(int N, int start, int target, int K) {
        ...
        /*
         ...
         注意：i的起始位置可以是1，但j的起始位置必须是0。因为i代表横坐标，而横坐标是起始位置，题目要求的起始位置是1，所以i从0或者从1开始都行，只不过0位置用不到而已。
         但是j位置必须从0开始，因为纵坐标代表的是剩余步数，剩余步数有可能是0，剩余步数的范围是[0,k]
         */
        for(int i = 0; i < N + 1; i++) {
            for(int j = 0; j < K + 1; j++) {
                buf[i][j] = -1;
    ...
    意思是当rest=0的时候（纵坐标为0时），只有cur=target的时候是1，其它都是0，所以[target][0]这个格子填1，其它是0
    ...
    普适的位置依赖于(cur - 1, rest - 1)和(cur + 1, rest - 1)，所以在二维矩阵中一个普适的格子依赖于它左下和右下两个格子，取这两个值的和，所以知道任意一个格子的左下和右下的值即可。
    特殊位置可以看做是普适位置的特例，比如当cur=1时，cur=1的所有列没有左下只有右下，那么就有什么算什么，就让当前位置等于它的右下就可以了；
    同理当cur=N时，cur=N的所有列没有右下只有左下。
    5、经过以上4个步骤所有各自都填满了，这一步就是回忆一下你填格子的顺序。首先，必须要在草稿纸上填满所有格子，然后看一下刚才填入的顺序：是固定某一个行去填满这一行上的所有列，
    ...
    第五步往往被忽视，但是很重要，因为它决定你遍历二维数组的顺序，比如本题就是先遍历列再遍历行，不能理所应当地认为先遍历行再遍历列。
     */
