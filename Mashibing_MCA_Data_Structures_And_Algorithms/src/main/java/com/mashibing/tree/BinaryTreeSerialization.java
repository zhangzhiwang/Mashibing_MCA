 ...
 * 使用递归的方式按照先序遍历的方式遍历二叉树，将原来的打印改为输出到队列中，这里需要注意的是为了占位需要把空孩子（null）也写进队列中。
 * 约定：字符串中元素之间使用逗号分割（由于输出的是Queue，所以默认就是以逗号分割的），null使用"#"代替。
 * 反序列化：
 * 用的也是递归，base case是从队列中弹出来的元素为"#"时返回null。
 * 另外，后续遍历的序列化和反序列化可以根据先序遍历的结果改出来，但是没有中序遍历的序列化和反序列化，因为中序遍历的序列化的结果会有歧义，不同的树序列化的结果可能是一样的，
 * 这样就没有办法反序列化了，所以中序遍历没有序列化和反序列化。
 * 课程：体系班课时91-92
 */
public class BinaryTreeSerialization {
    public static Queue<String> serializeBinaryTreePre(TreeNode<String> head) {
        if(head == null) {
            return null;// return一个空的Queue也行，看题目要求
        }
