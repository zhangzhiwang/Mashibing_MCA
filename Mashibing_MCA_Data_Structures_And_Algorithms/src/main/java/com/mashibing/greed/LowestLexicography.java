 ...
 * 解释：
 * 贪心算法：你提出一个能够解决局部问题的解决方案，这个方案推广到全局也能适用，即可以解决全局问题，这个方案就是贪心算法。
 ...
 * 所以在面试时先提出方案，然后直接写代码，写完代码写对数器，如果对数器验证不通过说明方案肯定不对，去想另一个方案，然后重复上面的过程，知道对数器验证通过为止。
 *
 * 所有贪心算法的题最终的代码都巨简单，难点在于你提出的局部问题的解正好也是全局问题的解，你的然心算法在全局问题上"能贪对"这个事巨难。
 *
 * 思路：写一个比较器，让数组相邻两个字符串做ascii比较，做正序排序，这比较时不是那相邻的两个字符转直接比较，而是先正反拼接之后再比较；比较完之后形成一个全新顺序的数组，
 * 然后顺序拼接即可，最终拼接成的大字符串就是字典序最小的。
