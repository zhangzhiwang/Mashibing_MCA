...
/**
 * 镜面树
 * 以整棵树的头结点位中心，将整棵树分成了左右两部分，左半部分和右半部分互为镜面（值相等位置相反）。这里要注意的是以整棵树左右两边互为镜面，不是任意节点的左右子树都互为镜面。
 * 
 * 思路：
 * 把一棵树以头结点为中心左右劈开，左子树的左节点和右子树的右节点比较，左子树的右节点和右子树的左节点比较，左右子树的每个节点的值一样结构也一样才行。
 * 一句话：以整棵树的头结点为中心画一条线，两边程轴对称就是镜面树。
 * 在实现上，判断一棵树是不是镜面树和比较两棵树是不是完全一样的代码是一样的，只不过前者是左和右比较、右和左比较，后者是左左比较、右右比较。
 * 
 * @author zhangzhiwang
