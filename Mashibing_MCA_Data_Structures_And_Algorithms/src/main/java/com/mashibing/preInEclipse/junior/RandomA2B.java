...
/**
 * 从a-b随机到c-d随机
 * 题目1：有一个给定的函数f()，它能等概率的返回1-5的随机整数，要求不能利用其它任何函数只能使用f()来等概率地生成1-7范围的整数。
 *
 * 思路：
 * 整体思路：
 * 1、将题目给出的已知范围（1-5）改出0、1等概率发生器
 * 2、将题目要求的范围（1-7）缩小成以0开头的范围：范围最大值和最小值都减去一个k，得到一个新的范围x
 * 3、分析新范围x的最大值需要几个二进制位来存放
 * 4、需要几个二进制位就调用几次等概率发生器，这样得到的结果的范围为y
 * 5、将范围y修正为范围x
 * 6、范围x加上k
 *
 * 具体分分析：
