 ...
 * 3、写一个总体的方法完成整个链表的反转：
 * （1）判断一下整个链表的长度是否达到了k，因为不满一组的不反转
 * （2）通过上面的判断之后开始循环反转每一组的链表，其中要有一个临时变量保存上一组反转后的end节点，注意这里要修正第二步反转后end节点的next指针的指向，
 * 之前是指向了下一组未反转的头结点，这回要修正为反转后的头结点
 * 
 * @author zhangzhiwang
...
public class ReverseNodesInKGroup {
	...
	public static Node reverseWholeListByKGroup(Node head, int k) {
		...
		// 判断整个链表的元素个数够不够k，如果k比整个链表的长度都长那么直接返回原始head
		Node start = head;
		...
		head = end;// 通过了上面的判断说明链表长度比k大，能够走到这里说明第一组的尾结点在下面被反转后会成为头结点，第一组的头结点也是整个链表的头结点，由于最终要返回新链表的头结点
					// 所以，这里要将头结点设置为第一组的尾结点用于最终返回，后续操作都不能用head节点来进行。
	...
	public static void reverseInGroup(Node start, Node end) {
		...
		Node pre = null;
		/*
		 这一步很关键，循环里面的操作用cur代替start来进行，循环退出后关键的一步是将原来的start的下一个指针指向下一组的开头，
		 也就是本次循环结束后end的位置，而且本方法从开始到结束根本就没有改变start的位置，外面还要用这个start。
		 */
		Node cur = start;
		Node next = null;
		end = end.next;// 这一步也很重要，因为在循环里面要操作end，也就是反转涉及到end，而判断条件是start != end，如果事先没有本步操作的话，当cur == end的时候就退出循环了，end操作不到
		while (cur != end) {
			next = cur.next;
			cur.next = pre;
			pre = cur;
