...
public class ByteOper {
	public static void main(String[] args) {
		 ...
		 * 3、为什么int的最大值不是2^31？
		 * 因为2^31表示在二进制中第32位（即索引为31的位置）的值是1，后面全是0，即2^31只能代表在32位中的最高位是1，后面全是0，二进制位：1000 0000 0000 0000 0000 0000 0000 0000，这显然不是最大值，最大值应该是各位都为1，比如：1111 1111 1111 1111 1111 1111 1111 1111。
		 * 是不是32位的所有位置全是1就代表最大值了？不是，因为最高位是符号位，0代表正数，1代表负数，所以真正的有效位只到第31位，也就是索引为30的位置。那2^30能表示吗？不能，前面已经说了，2^30只能表示第31位上的数字是1，后面全是0，即0100 0000 0000 0000 0000 0000 0000 0000。
		 * 排除第32位的符号位不看，正数部分的最大值应该是这个样子：0111 1111 1111 1111 1111 1111 1111 1111，由于每一位上的值都是1，所以换算成十进制就是：2^0 + 2^1 + 2^2 + …… + 2^30。那怎么表示这个最大值呢？最简单的方式就是让这个数+1，这样所有位都要往前进一位直到进到第32位，
		 ...
		 * 请看课程视频
		 * 所以int的范围是[-2^31,2^31-1]。
		 * 上面说这么多其实就是在解释2的n次方中的n为什么是31而不是32或者30，为什么负数边界没有减1，而正数边界要减1。
		 * 
		 * 之所以要保留一位当做符号位是因为int表示的是有符号整数，如果是无符号整数那么就可以用满32个位。所以负数最小值的绝对值要比整数最大值的绝对值大1，也就是能表示负数的个数要比能表示正数的个数多一个，如果把0归入正数范畴那么能表示正负数的个数就一样了，即正负数各有2^31个数，所以整体加起来一共有2^31 * 2 = 2^32个数。
		 *
