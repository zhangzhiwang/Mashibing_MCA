...
public class MergeSort {
	...
	private static void process(int[] arr, int l, int r) {
		/*
		base case：当左右数组只有一个元素就返回。base case什么都不需要返回，它的作用就是切割数组，将左右数组切割成只有一个元素为止。
		比如：
		1、原数组[0,3,1,7,5]，切成左数组[0,3,1]，右数组[7,5]；
		2、将上面的左数组进一步切分为[0,3]，右数组[1]，右数组已经是一个元素了，满足了base case，停止，左数组[0,3]继续切分为左数组[0]和右数组[3]，满足base case，停止；
		3、将第1步的右数组[7,5]进一步切分为左数组[7]和右数组[5]，满足base case，停止
		排序的时候是按照切分的顺序倒序排：
		1、原数组的左数组[0,3,1]，被进一步切分为：左：[0,3]，右：[1]；[0,3]切分为：左：[0]，右：[3]；
		最小粒度的左数组[0]只有一个元素，天然有序，同理最小粒度的右数组[3]天然有序，然后左右数组再排序，排完为[0,3]；右数组[1]天要有序，已经排完了，和左数组[0,3]排序，排完之后是[0,1,3]；
		2、同理，右数组[7,5]拆分到最小数组然后排完序是[5,7]；
		3、排序后的左数组[0,1,3]和排序后的右数组[7,5]，再进行排序，最终顺序就是[0,1,3,5,7]。
		 */
		if (l == r) {
			return;
		}
		 ...
		 * r和l之间差距的一半是多少呢？差距的一半是(r - l) / 2。比l大差距一半的数是多少呢？l加上一半的差距就是比l大差距一半的数，即l + (r - l) / 2。
		 * 把除法换成位运算就得到了：l + ((r - l) >> 1)
		 */
		// int m = (l + r) / 2;
		int m = l + ((r - l) >> 1);
		// 下面这两行递归的作用就是切割数组，将一个大数组切割成只剩下一个元素的数组
		process(arr, l, m);// 切割左数组：找到了中间位置之后，左数组的范围就是[l,m]
		process(arr, m + 1, r);// 切割右数组：右数组的范围就是[m+1,r]
