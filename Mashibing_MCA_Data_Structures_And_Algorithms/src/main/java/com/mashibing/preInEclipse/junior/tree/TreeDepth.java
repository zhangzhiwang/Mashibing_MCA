...
/**
 * 树的高度
 * 节点的高度：（从所在位置往下看才叫高度）该节点到叶子结点的最长路径（边数）。树的高度等于根节点的高度。
 * 节点的深度：（从所在位置往上看才叫深度）从根节点到该节点的边数。从上往下看，根节点到任意节点的通路只有一条，因为在树中不允许多个节点指向同一节点，所以不存在最长路径的说法。
 * 可以把整棵树比作大海，树的根节点也就是头结点root可以比作海面，叶子节点比作海底，所以某一个节点的深度是从海面（根节点）到改点的距离，而该节点的高度就是从改点到海底（叶子节点）的距离。
