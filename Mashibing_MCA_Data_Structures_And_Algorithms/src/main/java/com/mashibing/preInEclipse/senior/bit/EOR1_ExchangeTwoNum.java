...
//		System.out.println(a);
	...
	private static void exchange(int a, int b) {
		 ...
		 * 所以整体执行完a = B，b = A。
		 * 注意：
		 * 1、这种算法的前提条件是入参a和b的值可以相同，但是必须要指向两个不同的内存区域，如果引用a和b指向相同的内存区域，那么最后a和b都会被刷成0。
		 * 2、^运算只能用于以下基本数据类型及其包装类型：byte、short、char、int、long，其它基本数据类型和非基本数据类型都不能使用^运算否则编译不通过
		 */
		a = a ^ b;
		b = a ^ b;
		a = a ^ b;
